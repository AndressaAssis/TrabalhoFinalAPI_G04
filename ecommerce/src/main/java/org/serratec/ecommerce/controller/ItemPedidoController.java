...
public class ItemPedidoController {
	...
	@GetMapping
	@Operation(summary = "Retorna a lista de todos os pedidos da lista", description = "Dado a informação será listado todos o carrinho de pedidos.")
	@ApiResponses(value = {
	...
	public List<ItemPedidoResponseDto> listarTodos() {
		...
		return itens.stream()
				.map(item -> new ItemPedidoResponseDto(item.id(), item.pedidoId(), item.jogoId(), item.precoUnitario(),
						item.quantidade(), item.percentualDesconto(), item.valorBruto(), item.valorLiquido()))
				.collect(Collectors.toList());
	}

	@GetMapping("/{id}")
	@Operation(summary = "Retorna um carrinho de pedido pelo id", description = "Dado um determinado número de id, será retornado o carrinho do id correspondente.")
	@ApiResponses(value = {
	...
	@Operation(summary = "Cadastro de um novo carrinho", description = "Função de criação de um novo carrinho.")
	@ApiResponses(value = {
		...
		try {
			...
			if (dto == null) {
				throw new ResourceNotFoundException(
						"Informações inválidas fornecidas para criar o carrinho: objeto não pode ser nulo.");
			}
			if (dto.pedidoId() == null || dto.jogoId() == null) {
				throw new ResourceNotFoundException(
						"Informações inválidas fornecidas para criar o carrinho: 'pedidoId' e 'jogoId' são obrigatórios.");
			}
	...
	@DeleteMapping("/{id}")
	@Operation(summary = "Deleta um carinho pelo id", description = "Dado um determinado número de id, ele irá excluir o carrinho do id correspondente.")
	@ApiResponses(value = { @ApiResponse(responseCode = "400", description = "Carrinho não localizado. Verifique!"),
	...
	@PutMapping("/{id}")
	@Operation(summary = "Atualiza um carrinho pelo id", description = "Dado um determinado número de id, ele irá modificar o carrinho do cliente pelo id correspondente.")
	@ApiResponses(value = { @ApiResponse(responseCode = "400", description = "Carrinho não localizado. Verifique!"),
	...
	public ResponseEntity<ItemPedidoDto> atualizarItemPedido(@PathVariable Long id, @RequestBody ItemPedidoDto dto) {
		try {
			...
			if (dto == null) {
				throw new ResourceNotFoundException(
						"Informações inválidas fornecidas para atualizar o carrinho: objeto não pode ser nulo.");
			}
			if (dto.pedidoId() == null || dto.jogoId() == null) {
				throw new ResourceNotFoundException(
						"Informações inválidas fornecidas para atualizar o carrinho: 'pedidoId' e 'jogoId' são obrigatórios.");
			}
	...
	@Operation(summary = "Retorna a lista do carrinho de acordo com o pedido.", description = "Dado a informação será retornado o carrinho de acordo com o pedido do id.")
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Caso a lista retorne vazia, é porque não existe nenhum pedido ao carrinho.") })
	...
	@Operation(summary = "Retorna a lista do carrinho de acordo com o jogo.", description = "Dado a informação será retornado o carrinho que tem todos os jogos referentes ao id.")
	@ApiResponses(value = {
